...
--[[
	...
	- Credits Thing (Done)
	- Readd Secret (Done)
	- Shiba Hub Support (Official 1.5 Release)
]]
--[[ Ownership ]] --
...
-- [[ Optimizer ]] --
local HideRealChar = Config.HideRealChar or false -- Hides Real Character, Gives +10 FPS Perfomance
...
do --[[ Bullet/TorsoFling Checking ]]--
	...
	elseif IsBulletEnabled == true and RigType == "R15" then
		if not Character:FindFirstChild("SniperShoulderL") then -- [[ Hat Check ]] -- 
			...
			FakeHat.Parent = Character
			BulletHatInfo = {FakeHat, Character:FindFirstChild("LeftUpperArm"), CFrame.new(), Vector3.new(), Vector3.new()}
		else
...
do -- [[ Optimization ]] --
	if HideRealChar == true then
		...
		if BulletPartInfo then
			BulletPartInfo[1].Size = ExtraThing
		end
		FakeRig.HumanoidRootPart.Transparency = 1
		ExtraThing = nil
		local NewColors = Character:FindFirstChildWhichIsA("BodyColors")
		...
		if NewColors then
			if FakeRig:FindFirstChildWhichIsA("BodyColors") then
				FakeRig:FindFirstChildWhichIsA("BodyColors"):Destroy()
			end
			local New = NewColors:Clone()
			New.Parent = FakeRig
		end
		if Shirt then
			if FakeRig:FindFirstChildWhichIsA("Shirt") then
				FakeRig:FindFirstChildWhichIsA("Shirt"):Destroy()
			end
			local New = Shirt:Clone()
			New.Parent = FakeRig
		end
		if Pants then
			if FakeRig:FindFirstChildWhichIsA("Pants") then
				FakeRig:FindFirstChildWhichIsA("Pants"):Destroy()
			end
			local New = Pants:Clone()
			New.Parent = FakeRig
		end
		if TShirt then
			if FakeRig:FindFirstChildWhichIsA("ShirtGraphic") then
				FakeRig:FindFirstChildWhichIsA("ShirtGraphic"):Destroy()
			end
			local New = TShirt:Clone()
			New.Parent = FakeRig
		end
		FakeRig.Head.face.Texture = Face.Texture
	end
end
...
table.insert(Events, Player.Chatted:Connect(function(Text)
	if Text == "gelatek skid" or Text == "i love south park" or Text == "kyle feet" or Text == "i use align" then
